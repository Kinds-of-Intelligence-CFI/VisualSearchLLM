def constructMessage(writing, shape, image, model, finetuning=False, solution=None):
    prompts = { "0": f"Which quadrant is the {shape} in this image located? In the case where the {shape} overlaps multiple quadrants, please provide the quadrant where the centre of the {shape} is located. Please only respond with 'Quadrant i' where i is the quadrant you think the {shape} is in. 1 is the top left, 2 is the top right, 3 is the bottom left, 4 is the bottom right. Do not reply with anything else.",
               ...
               "std2x2-2Among5": f"The image is divided into a 2x2 grid. Each element of the grid is referred to as a cell. In the image there are a number of objects. Almost all of the objects are the number 5. There is a single 2 in the image. In which cell is the 2 in? In the case where the 2 overlaps multiple cells, please provide the cell where the centre of the 2 is located. Please only respond with 'Cell (i,j)' where (i,j) corresponds to the ith row and jth column of the grid. The top left cell is Cell (1,1). Do not reply with anything else." 
              }
    message = []
