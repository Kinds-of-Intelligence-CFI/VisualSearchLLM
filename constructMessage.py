def constructMessage(writing, shape, colour, image, model, finetuning=False, solution=None):
    prompts = { "0": f"Which quadrant is the {shape} in this image located? In the case where the {shape} overlaps multiple quadrants, please provide the quadrant where the centre of the {shape} is located. Please only respond with 'Quadrant i' where i is the quadrant you think the {shape} is in. 1 is the top left, 2 is the top right, 3 is the bottom left, 4 is the bottom right. Do not reply with anything else.",
               ...
               "std2x2-2Among5-conj": f"THe image is divided into a 2x2 grid. Each element of the grid is referred to as a cell. In the presented image there are a number there are a number of objects. There are '2's and '5's written as numerals. In which cell is the {colour} '{shape}'? In the case where the {colour} {shape} overlaps multiple cells, please provide the cell where the centre of the {shape} is located. Please only respond with 'Cell (i,j)' where (i,j) corresponds to the ith row and jth column of the grid. The top left cell is Cell (1,1). If you are uncertain you may optionally add a note explaining that but please start your response with 'Cell (i,j)'."
              }
    message = []
